...
func _on_data_subscribed(subscribe: bool, channel_name: String) -> void:
	...
	if (subscribe):
		...
		if (channel_name in self._selected_channels_name):
			self.log(LOG_DEBUG, ["Unsubscribing for: ", channel_name, "[Current subscribed channel: ", self._selected_channels_name, "]"])
			self._selected_channels_name.erase(channel_name)
			self.log(LOG_DEBUG, ["After unsubscribing for: ", channel_name, "[Current subscribed channel: ", self._selected_channels_name, "]"])
