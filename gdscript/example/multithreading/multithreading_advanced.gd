...
var _guidot_client: Guidot_Data_Client
...
func _ready() -> void:
	...
	self._guidot_client = self.get_tree().get_nodes_in_group(Guidot_Common._client_group_name)[0]
...
func _process(delta: float) -> void:
	...
	self._guidot_server.register_client(self._guidot_client)
	print(self._guidot_server.query_data_with_channel_name("mouse_x"))
